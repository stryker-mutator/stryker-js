...
async function buildMonoSchema() {
  const schemaFiles = await glob('packages/!(core)/schema/*.json', {
    cwd: resolveFromParent(),
  });
  const allContent = await Promise.all(
    schemaFiles.map((schemaFile) =>
      readFile(resolveFromParent(schemaFile), 'utf8'),
    ),
  );
  ...
  const monoSchema = {
    ...
    description:
      'Options for Stryker for JS and TypeScript and all officially supported plugins.',
    type: 'object',
    properties: allSchemas.reduce(
      (props, schema) => ({ ...props, ...schema.properties }),
      {},
    ),
    definitions: allSchemas.reduce(
      (props, schema) => ({ ...props, ...schema.definitions }),
      {},
    ),
  };
  const outFile = resolveFromParent(
    'packages',
    'core',
    'schema',
    'stryker-schema.json',
