...
export class HtmlReporter implements Reporter {
  ...
  public onAllMutantsTested(): void {
    this.#mutationEventServer.sendFinished();
  }
  ...
  private async generateReport(report: schema.MutationTestResult, sseUrl?: string) {
    this.log.debug(`Using file "${this.options.htmlReporter.fileName}"`);
    const html = await createReportHtml(report, sseUrl);
...
async function createReportHtml(report: schema.MutationTestResult, sseUrl?: string): Promise<string> {
  ...
  <body>
    ...
    <mutation-test-report-app titlePostfix="Stryker" ${sseUrl ? `sse=${sseUrl}` : ''} sse="${sseUrl}" }>
