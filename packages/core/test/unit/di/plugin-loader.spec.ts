...
describe(PluginLoader.name, () => {
  ...
  it('should load local plugins using their file URLs', async () => {
    ...
    const expectedModuleReporters = pathToFileURL(
  ...
  it('should load plugins matching a wildcard, ignoring "util", "api", "core" and "instrumenter"', async () => {
    const fooPlugin: SchemaValidationContribution = {
      strykerValidationSchema: { foo: 'plugin' },
    };
  ...
  it('should return the plugin module paths of modules that contributed plugins', async () => {
    ...
    const barPlugin: PluginModule & SchemaValidationContribution = {
      strykerValidationSchema: { bar: 'plugin' },
      strykerPlugins: [],
    };
    const bazPlugin = {};
    importModuleStub
  ...
  it('should return plugins grouped by kind', async () => {
    ...
    const fooPlugin: PluginModule = {
      strykerPlugins: [expectedReporters[0], expectedCheckers[0]],
    };
    const bazPlugin: PluginModule = {
      strykerPlugins: [expectedReporters[1], expectedCheckers[1]],
    };
    importModuleStub
      .withArgs('foo')
      .resolves(fooPlugin)
    ...
    // Assert
    expect([...result.pluginsByKind.keys()]).deep.eq([
      PluginKind.Reporter,
      PluginKind.Checker,
    ]);
    expect(result.pluginsByKind.get(PluginKind.Reporter)).deep.eq(
      expectedReporters,
    );
    expect(result.pluginsByKind.get(PluginKind.Checker)).deep.eq(
      expectedCheckers,
    );
  });
  ...
  it('should return the schema contributions', async () => {
    ...
    // Assert
    expect(result.schemaContributions).deep.eq([
      { foo: 'plugin' },
      { bar: 'plugin' },
    ]);
  });
  ...
  it('should not allow non-object schema contributions', async () => {
    importModuleStub.resolves({
      strykerValidationSchema: 'error, should be an object',
    });
  ...
  it('should log ERR_MODULE_NOT_FOUND errors for files other than the file to load as "error during loading..." warnings', async () => {
    ...
    expect(testInjector.logger.warn).calledWithExactly(
      ...
      'b',
      "Cannot find package 'a' imported from d.js",
    );
  });
