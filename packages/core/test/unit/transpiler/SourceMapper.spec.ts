...
import { MAPPED_LOCATION, Mock, mock } from '../../helpers/producers';

const greatestLowerBound = sourceMapModule.SourceMapConsumer.GREATEST_LOWER_BOUND;
const leastUpperBound = sourceMapModule.SourceMapConsumer.LEAST_UPPER_BOUND;

function base64Encode(input: string) {
  return Buffer.from(input).toString('base64');
}

const errorPostfix = '. Cannot analyse code coverage. Setting `coverageAnalysis: "off"` in your stryker.conf.js will prevent this error, but forces Stryker to run each test for each mutant.';
...
describe('SourceMapper', () => {
  ...
  beforeEach(() => {
    ...
    sourceMapModule.SourceMapConsumer.LEAST_UPPER_BOUND = leastUpperBound;
    sourceMapModule.SourceMapConsumer.GREATEST_LOWER_BOUND = greatestLowerBound;
  });
  ...
  describe('Transpiled', () => {
    ...
    it('should create SourceMapConsumer for a file when transpiledLocationFor is called', async () => {
      ...
      // Act
      await sut.transpiledLocationFor(MAPPED_LOCATION({ fileName: 'file1.ts' }));
    ...
    it('should throw an error when the requested source map could not be found', async () => {
      await expect(sut.transpiledLocationFor(MAPPED_LOCATION({ fileName: 'foobar' }))).to.be.rejectedWith(SourceMapError, 'Source map not found for "foobar"' + errorPostfix);
    });
    ...
    it('should throw an error if source map file is a binary file', async () => {
      ...
      await expect(sut.transpiledLocationFor(MAPPED_LOCATION({ fileName: 'foobar' })))
    ...
    it('should throw an error if source map data url is not supported', async () => {
      ...
      await expect(sut.transpiledLocationFor(MAPPED_LOCATION({ fileName: 'foobar' })))
        .to.be.rejectedWith(SourceMapError, `Source map file for "file1.js" cannot be read. Data url "data:application/xml;base64" found, where "data:application/json;base64" was expected${errorPostfix}`);
    });
    ...
    it('should throw an error if source map file cannot be found', async () => {
      ...
      await expect(sut.transpiledLocationFor(MAPPED_LOCATION({ fileName: 'foobar' })))
        .to.be.rejectedWith(SourceMapError, `Source map file "file1.js.map" (referenced by "file1.js") cannot be found in list of transpiled files${errorPostfix}`);
    });
    ...
    it('should throw an error if source map file url is not declared in a transpiled file', async () => {
      ...
      await expect(sut.transpiledLocationFor(MAPPED_LOCATION({ fileName: 'foobar' })))
        .to.be.rejectedWith(SourceMapError, `Source map not found for "foobar"${errorPostfix}`);
    });
    ...
    it('should not throw an error if one of the files is a binary file', async () => {
      ...
      await expect(sut.transpiledLocationFor(MAPPED_LOCATION({ fileName: 'file1.ts' }))).to.eventually.deep.eq({
        fileName: 'file1.js',
        location: {
          end: {
            column: 2,
