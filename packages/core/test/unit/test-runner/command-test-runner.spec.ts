...
describe(CommandTestRunner.name, () => {
  ...
  beforeEach(() => {
    ...
    execMock = sinon
      ...
      .returns(childProcessMock as childProcess.ChildProcess);
        ...
        tests: [
          {
            ...
            timeSpentMs: 0,
            failureMessage: `x Test 1 failed${os.EOL}1 != 2`,
          },
        ],
      };
      expect(result).deep.eq(expectedResult);
  ...
  function actDryRun(
    sut: CommandTestRunner = createSut(),
    exitCode = 0,
    elapsedTimeMS = 0,
  ) {
    const resultPromise = sut.dryRun(factory.dryRunOptions());
  ...
  function actMutantRun(
    sut: CommandTestRunner = createSut(),
    { exitCode = 0, activeMutantId = '0' },
  ) {
    const resultPromise = sut.mutantRun({
      activeMutant: factory.mutant({ id: activeMutantId }),
    });
    actTestProcessEnds(exitCode);
    return resultPromise;
  }
