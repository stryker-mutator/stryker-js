...
describe(`${KarmaTestRunner.name} integration`, () => {
  ...
  describe('when an error occurs while running tests', () => {
    ...
    describe('dryRun', () => {
      it('should report Error with the error message', async () => {
        ...
        expect(runResult.errorMessage).include(
          'ReferenceError: someGlobalVariableThatIsNotDeclared is not defined',
        );
      });
    });
  ...
  describe('when no error occurred and no test is performed', () => {
    before(() => {
      setOptions({
        files: [
          'testResources/sampleProject/src/Add.js',
          'testResources/sampleProject/test-jasmine/EmptySpec.js',
        ],
      });
      sut = createSut();
      return sut.init();
    ...
    before(() => {
      setOptions({
        files: [
          ...
          {
            pattern: 'testResources/sampleProject/test-jasmine/AddSpec.js',
            included: true,
          },
          {
            pattern: 'testResources/sampleProject/src/Error.js',
            included: false,
          },
        ],
      });
  ...
  describe('when specified port is not available', () => {
    ...
    it('should choose different port automatically and report Complete without errors', async () => {
      const actualResult = await sut.dryRun(factory.dryRunOptions());
      expect(StrykerReporter.instance.karmaConfig!.port).not.eq(
        dummyServer.port,
      );
      assertions.expectCompleted(actualResult);
    });
...
class DummyServer {
  ...
  public get port() {
    ...
    if (!address || typeof address === 'string') {
      throw new Error(
        `Address "${address}" was unexpected: https://nodejs.org/dist/latest-v11.x/docs/api/net.html#net_server_address`,
      );
    } else {
      return address.port;
    }
  ...
  private async init(): Promise<void> {
    const listen: (port: number, host: string) => Promise<void> = promisify(
      this.httpServer.listen.bind(this.httpServer),
    );
