...
describe('svelte-printer', () => {
  ...
  it('should replace single script tag', () => {
    ...
    const expectedOutput =
      /<script>.*const name = "test";.*<\/script><h1>hello!<\/h1>/s;
    const ast = createSvelteAst({
      rawContent: input,
      root: {
        moduleScript: {
          ast: script,
          range: { start: script.root.start, end: script.root.end! },
          isExpression: false,
        },
        additionalScripts: [],
      },
  ...
  it('should replace multiple script tags and binding', () => {
    ...
    const scripts = [
      ...
      createTemplateScript({
        ...
        range: createRange(43, 44),
      }),
      createTemplateScript({
        ast: createJSAst({ rawContent: '4' }),
        range: createRange(99, 100),
      }),
      createTemplateScript({
        ast: createJSAst({ rawContent: '3' }),
        range: createRange(81, 82),
      }),
      createTemplateScript({
        ast: createJSAst({ rawContent: '5' }),
        range: createRange(110, 111),
        isExpression: true,
      }),
    ];
    contextStub.print
    ...
    const input =
      '<script>1</script><script context="module">2</script><div><h1>hello!</h1><script>3</script><script>4</script>{5}</div>';
    ...
    const ast = createSvelteAst({
      rawContent: input,
      root: { moduleScript: scripts[0], additionalScripts: scripts.slice(1) },
    });
