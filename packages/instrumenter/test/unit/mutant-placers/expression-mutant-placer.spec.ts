...
describe('expressionMutantPlacer', () => {
  ...
  describe(expressionMutantPlacer.canPlace.name, () => {
    ...
    it('should be false when the parent is tagged template expression', () => {
      ...
      const templateLiteral = findNodePath(parseJS('html`<p></p>`'), (p) =>
        p.isTemplateLiteral(),
      );
    ...
    it('should be false when the parent is a delete unary expression', () => {
      const memberExpression = findNodePath(
        parseJS('delete myVariable?.[indexer];'),
        (p) => p.isOptionalMemberExpression(),
      );
    ...
    it('should be true when the parent is a non-delete unary expression', () => {
      const memberExpression = findNodePath(
        parseJS('void myVariable[indexer];'),
        (p) => p.isMemberExpression(),
      );
      const memberExpression2 = findNodePath(
        parseJS('typeof myVariable[indexer];'),
        (p) => p.isMemberExpression(),
      );
      const memberExpression3 = findNodePath(
        parseJS('throw myVariable[indexer];'),
        (p) => p.isMemberExpression(),
      );
    ...
    describe('object literals', () => {
      it('should be false when the expression is a key', () => {
        ...
        const stringLiteral = findNodePath(
          parseJS("const foo = { 'foo': bar }"),
          (p) => p.isStringLiteral(),
        );
      ...
      it('should be true when the expression is the value', () => {
        ...
        const stringLiteral = findNodePath(
          parseJS("const foo = { 'foo': bar }"),
          (p) => p.isIdentifier() && p.node.name === 'bar',
        );
    ...
    describe('chain expressions', () => {
      type ChainExpressionArrangement = [
        code: string,
        selector: (path: NodePath) => boolean,
        only?: true,
      ];
      ...
      const okPointers: ChainExpressionArrangement[] = [
        ...
        [
          ...
          (p) =>
            p.isCallExpression() &&
            types.isIdentifier(p.node.callee, { name: 'bar' }),
        ],
        [
          'foo(bar());',
          (p) =>
        ...
        [
          ...
          (p) =>
            ...
            types.isIdentifier(p.node.object, { name: 'bar' }),
        ],
      ...
      const falsePointers: ChainExpressionArrangement[] = [
        [
          ...
          (p) =>
            p.isMemberExpression() &&
            types.isIdentifier(p.node.property, { name: 'bar' }),
        ],
        [
          'foo.bar()',
          (p) =>
  ...
  describe(expressionMutantPlacer.place.name, () => {
    ...
    it('should place the original code as the alternative', () => {
      const { binaryExpression, appliedMutants, ast } = arrangeSingleMutant();
      expressionMutantPlacer.place(binaryExpression, appliedMutants);
      const actualAlternative = findNodePath<babel.types.ConditionalExpression>(
        ast,
    ...
    it('should add mutant coverage syntax', () => {
      ...
      expect(
        ...
        `${actualAlternativeCode} did not start with "${expected}"`,
      ).true;
    });
    ...
    it('should be able to place multiple mutants', () => {
      ...
      // Assert
      expect(actualCode).contains(
        'const foo = stryMutAct_9fa48("659") ? bar : stryMutAct_9fa48("52") ? bar - baz',
      );
    });
    ...
    function arrangeSingleMutant() {
      ...
      const appliedMutants = new Map([
        [mutant, mutant.applied(binaryExpression.node)],
      ]);
    ...
    describe('anonymous expressions', () => {
      function arrangeActAssert(
        ast: babel.types.File,
        expression: NodePath<babel.types.Expression>,
        ...
        const appliedMutants = new Map([
          [mutant, mutant.applied(expression.node)],
        ]);
      ...
      it('should set the name of an anonymous function expression', () => {
        ...
        const functionExpression = findNodePath<babel.types.FunctionExpression>(
          ast,
          (p) => p.isFunctionExpression(),
        );
        arrangeActAssert(
          ast,
          functionExpression,
      ...
      it('should set the name of an anonymous class expression', () => {
        ...
        const classExpression = findNodePath<babel.types.ClassExpression>(
          ast,
          (p) => p.isClassExpression(),
        );
      ...
      it('should set the name of an anonymous arrow function', () => {
        ...
        const functionExpression =
          findNodePath<babel.types.ArrowFunctionExpression>(ast, (p) =>
            p.isArrowFunctionExpression(),
          );
        arrangeActAssert(
          ast,
          functionExpression,
          /const bar =.*\(\(\) => { const bar = \(\) => {}; return bar; }\)\(\)/,
        );
      });
    });
  });
