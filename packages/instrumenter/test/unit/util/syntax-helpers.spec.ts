...
describe('syntax-helpers', () => {
  describe('instrumentationBabelHeader', () => {
    it('should be immutable', () => {
      ...
      expect(syntaxHelpers.instrumentationBabelHeader[0].leadingComments)
        .frozen;
    });
  });
  ...
  describe('isTypeNode', () => {
    ...
    it('should identify type annotations', () => {
      const input = findNodePath(parseTS('const foo: bar = baz;'), (p) =>
    ...
    it('should identify type alias declarations', () => {
      const input = findNodePath(parseTS('type Foo = "bar"'), (p) =>
    ...
    it('should identify TS module declarations', () => {
      const input = findNodePath(parseTS('declare module "express" {}'), (p) =>
    ...
    it('should be false for module (without `declare`)', () => {
      const input = findNodePath(parseTS('namespace A { }'), (p) =>
        p.isTSModuleDeclaration(),
      );
    ...
    it('should identify TS enum declarations', () => {
      const input = findNodePath(parseTS('enum A { B }'), (p) =>
    ...
    it('should identify TS declare function syntax', () => {
      const input = findNodePath(
        parseTS('declare function foo(): void;'),
        (p) => p.isTSDeclareFunction(),
      );
      expect(syntaxHelpers.isTypeNode(input)).true;
    });

    it('should identify TS generic type parameter instantiations', () => {
      const input = findNodePath(parseTS('foo<"test">()'), (p) =>
    ...
    it('should identify TS generic type parameters declarations', () => {
      const input = findNodePath(
        parseTS('function foo<bar extends `${position} ${color}`>() { }'),
        (p) => p.isTSTypeParameterDeclaration(),
      );
      expect(syntaxHelpers.isTypeNode(input)).true;
    });
  });
