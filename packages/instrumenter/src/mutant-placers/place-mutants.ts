 ...
 * @param nodePath The ast node to try and replace with a mutated
 ...
 */
export function placeMutants(
  nodePath: NodePath,
  mutants: Mutant[],
  fileName: string,
  mutantPlacers: readonly MutantPlacer[] = MUTANT_PLACERS
): types.Node | undefined {
  if (mutants.length) {
    for (const placer of mutantPlacers) {
      try {
        const node = placer(nodePath, mutants);
        if (node) {
          return node;
        }
      } catch (error) {
