...
import { type NodeMutator } from '../mutators/index.js';
...
export const createTransformBabel = Object.assign(
  (mutators: NodeMutator[]): AstTransformer<ScriptFormat> =>
    (
       ...
       */
      function registerInPlacementMap(
        path: NodePath,
        mutantsToPlace: Mutant[],
      ) {
      ...
      function placeMutantsIfNeeded(path: NodePath) {
        ...
        try {
          if (mutantsPlacement?.appliedMutants.size) {
            mutantsPlacement.placer.place(
              path,
              mutantsPlacement.appliedMutants,
            );
            path.skip();
          }
        } catch (error) {
  ...
  {
    inject: [instrumenterTokens.mutators] as const,
  },
);
