...
import BroadcastReporter, {
  NamedReporter
} from './reporters/BroadcastReporter';
...
export default class ReporterOrchestrator {
  ...
  public createBroadcastReporter(): StrictReporter {
    ...
    if (reporterOption && reporterOption.length) {
      reporterOption.forEach(reporterName =>
        reporters.push(this.createReporter(reporterName))
      );
    } else {
      this.log.warn(
  ...
  private createReporter(name: string) {
    if (name === 'progress' && !process.stdout.isTTY) {
      ...
      return {
        name,
        reporter: ReporterFactory.instance().create(name, this.options)
      };
    }
  }
