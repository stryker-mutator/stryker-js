...
export default class Sandbox {
  ...
  constructor(private options: Config, private index: number, private files: InputFile[], private testFramework: TestFramework | null, private coverageInstrumenter: CoverageInstrumenter | null) {
    this.workingFolder = StrykerTempFolder.instance().createRandomFolder('sandbox');
  ...
  private fillSandbox(): Promise<void[]> {
    ...
    if (this.coverageInstrumenter) {
      copyPromises.push(StrykerTempFolder.instance().writeFile(this.testHooksFile, this.coverageInstrumenter.hooksForTestRun()));
    } else {
      copyPromises.push(StrykerTempFolder.instance().writeFile(this.testHooksFile, ''));
    }
  ...
  private copyFile(file: InputFile): Promise<void> {
    if (isOnlineFile(file.path)) {
      ...
      return StrykerTempFolder.instance().copyFile(file.path, targetFile, instrumentingStream);
    }
  }
  ...
  private filterTests(mutant: Mutant) {
    if (this.testFramework) {
      ...
      return StrykerTempFolder.instance().writeFile(this.testHooksFile, fileContent);
    } else {
