...
import { createJasmineTestRunnerFactory } from '../../src/index.js';
...
async function main() {
  ...
  async function doDryRun(n = 40) {
    if (n > 0) {
      ...
      const result = await jasmineRunner.dryRun(factory.dryRunOptions({ coverageAnalysis: 'off', timeout: 3000 }));
