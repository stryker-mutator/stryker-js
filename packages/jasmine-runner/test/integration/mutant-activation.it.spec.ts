...
describe(`${JasmineTestRunner.name} mutant activation`, () => {
  ...
  it('should support static', async function () {
    ...
    expect(result.failureMessage.startsWith('Error:'), result.failureMessage)
      .true;
    expect(
      JSON.parse(result.failureMessage.substring('Error:'.length)),
    ).deep.eq({ runtimeActiveMutant: '42', staticActiveMutant: '42' });
  });
  ...
  it('should support runtime', async function () {
    ...
    expect(
      JSON.parse(result.failureMessage.substring('Error:'.length)),
    ).deep.eq({ runtimeActiveMutant: '42', staticActiveMutant: null });
  });
});
