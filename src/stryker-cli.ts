...
program
  ...
  .option('--test-framework <name>', `The name of the test framework you want to use`)
  .option('--test-runner <name>', `The name of the test runner you want to use`)
  .option('--test-selector <name>', `The name of the test selector you want to use`)
  ...
  .option('--timeout-ms <number>', 'Tweak the absolute timeout used to wait for a test runner to complete', parseInt)
  .option('--timeout-factor <number>', 'Tweak the standard deviation relative to the normal test run of a mutated test', parseFloat)
  .option('--plugins <listOfPlugins>', 'A list of plugins you want stryker to load (`require`).', list)
  .option('-c, --config-file <configFileLocation>', 'A location to a config file. That file should export a function which accepts a "config" object\n' +
  CONFIG_SYNTAX_HELP)
  .option('--log-level <level>', 'Set the log4js loglevel. Possible values: fatal, error, warn, info, debug, trace, all and off. Default is "info"')
  .parse(process.argv);
