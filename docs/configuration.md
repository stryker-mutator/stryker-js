...
This is useful to speed up Stryker by ignoring big directories/files you might have in your repo that has nothing to do with your code. For example, 1.5GB of movie/image files.
...
These patterns are **always ignored**: `['node_modules', '.git', '/reports' '*.tsbuildinfo', '/stryker.log', '.stryker-tmp']`. Because Stryker always ignores these, you should rarely have to adjust the `"ignorePatterns"` setting at all.
...
### `ignorePatterns` with `"!"` (= undo)



If you want to undo one of these ignore patterns, you can use the `!` prefix, for example:

- `--ignorePatterns "src/**","!str/app/important/*.ts"` (you need to use quotes on the command line , if you use patterns with `*`, `**` or [others](#configuration-options-and-globbing-expressions))


- or in the config file: `"ignorePatterns": ["src/**","!str/app/important/*.ts"]`

This would ignore everything in and below `src` - directory **except** the typescript files in `src/app/important` directory, but the `--mutate` might be the better option in that case , see [below](#mutate-string)

Keep in mind that you should **not accidentally ignore any other configuration** files your test runner might need for running the tests in the sandbox directory. (except you use the [inPlace](#inplace-boolean) option )
...
Command line: `[--mutate|-m] "src/**/*.js","a.js"` (You need to use quotes on the command line , if you use patterns with `*` or `**` in order to prevent the shell from expanding them before stryker can read them, [see also](#configuration-options-and-globbing-expressions))<br />


We suggest using [https://globster.xyz/](https://globster.xyz/) when auditing more complex glob with `*` or `**` expressions; it can help you get them just right.

- Config file: `"mutate": ["src/**/*.js", "a.js"]`
- command line: `--mutate "src/**/*.js", "a.js"` or `-m "src/\*_/_.js", "a.js"`
  `
...
(Whereas with [`ignorePatterns`](#ignorepatterns-string) you prevent non-relevant files from beeing copied to the [sandbox directory](#tempDirName-string) in the first place)
...
If the defaults are not sufficient for you, for example in a angular project you might want to **exclude** not only the `*.spec.ts` files but other files too, you can use : <br>

- `--mutate "src/**/*.ts","!src/**/*.spec.ts","!src/**/*.module.ts"` on command line, (you need to use quotes on the command line , if you use patterns with `*` or `**` )
- `"mutate": ["src/**/*.ts","!src/**/*.spec.ts","!src/**/*.module.ts"]` in the config file

Keep in mind that each command line option always **overwrite** the respective config file parameter!
...
**Other examples:**
If you want to only mutate one specific file:

- `--mutate src/app/home/home.component.ts`, or if you want to mutate just one directory:
- `--mutate "src/app/home/*.ts","!src/app/home/*.spec.ts"`
