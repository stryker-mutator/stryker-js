...
describe('Verify stryker has ran correctly', () => {
  ...
  it('should not report the mutant run', () => {
    expect(
      ...
      'Expected no reports to be written to disk, but they did',
    ).false;
  });
  it('should not delete the temp dir', () => {
    expect(
      fs.existsSync('.stryker-tmp'),
      'Expected the `.stryker-tmp` dir to not be deleted.',
    ).true;
  });
});
