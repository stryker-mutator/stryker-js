...
import { execStryker } from '../../../helpers.js';

describe('Options validation', () => {
  /**
   * @type {import('../../../helpers.js').ExecStrykerResult}
   */
  let result;
  describe('Errors in plugin options', () => {
    before(() => {
      result = execStryker(
        'stryker run stryker-error-in-plugin-options.conf.json',
      );
    });
    ...
    it('should report mochaOptions.spec error', () => {
      expect(result.stdout).includes(
        'Config option "mochaOptions.spec" has the wrong type',
      );
    });
    it('should report jasmineConfigFile error', () => {
      expect(result.stdout).includes(
        'Config option "jasmineConfigFile" has the wrong type',
      );
    });
    it('should report karma.projectType error', () => {
      expect(result.stdout).not.includes(
        'Config option "karma.projectType" has the wrong type',
      );
      expect(result.stdout).includes(
        'Config option "karma.projectType" should be one of the allowed values',
      );
    });
  });
  describe('Warnings ', () => {
    ...
    it('should report about unknown options', () => {
      expect(result.stdout).includes(
        'Unknown stryker config option "myCustomReporter"',
      );
    });
