...
function runE2eTests() {
  const { positionals, values } = parseArgs({
    ...
    options: {
      ...
      concurrency: {
        type: 'string',
        default: Math.max(os.cpus().length / 2, 2).toString(),
      },
    },
  ...
  /** @type {string[]} */
  const done =
    values.continue && fs.existsSync(progressFile)
  ...
  console.log(
    `Running e2e test in ${testDirs.length ? testDirs.join(', ') : '<none>'}`,
  );
    ...
    tap({
      next: (testDir) => {
        if (!abortController.signal.aborted) {
          ...
          console.log(
            `\u2714 ${testDir} tested (${++testsRan}/${testDirs.length})`,
          );
        }
      },
      error: () => {
...
function satisfiesNodeVersion(pkg) {
  ...
  if (
    ...
    !semver.satisfies(process.version, supportedNodeVersionRange)
  ) {
    console.log(
      `(node version ${process.version} did not satisfy ${supportedNodeVersionRange})`,
    );
    return false;
  } else {
    return true;
...
class TestFailedError extends Error {
  ...
  constructor(testDir, cause) {
    super(
      `Test "${testDir}" failed${cause instanceof Error ? `: ${cause.message}` : ''}`,
      { cause },
    );
...
async function runTest(testDir, signal, count = 0) {
  ...
  const pkg = JSON.parse(
    fs.readFileSync(
      path.resolve(testRootDir, testDir, 'package.json'),
      'utf-8',
    ),
  );
