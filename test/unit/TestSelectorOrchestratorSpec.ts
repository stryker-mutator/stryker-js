...
describe('TestSelectorOrchestrator', () => {
  ...
  describe('when options does not contain a testFramework', () => {
    describe('and testSelector is not set', () => {
      ...
      it('should log a warning for the missing setting', () => expect(logger.warn).to.have.been.calledWith('Missing config settings `testFramework` or `testSelector`. Stryker will continue without the ability to select individual tests, thus running all test for every generated mutant. Set `testSelector` option explicitly to `null` to ignore this warning.'));
